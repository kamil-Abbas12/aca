...
export default function DoctorVisitsPage() {
  ...
  const options = [
    ...
    { value: "8", label: "8 visits per year" },
    { value: "8+", label: "More than 8 visits per year" },
  ];

  const handleSelect = (value: string) => {
    setSelected(value);
router.push(`/monthly-income?doctor_visits=${value}`);
  };
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-12">
          ...
          <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full w-[65%] bg-[#1e2a4a] rounded-full" />
          </div>
        </div>
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[#1e2a4a] text-center mb-10 flex items-center justify-center gap-2 flex-wrap">
          How many times do you estimate you visit the doctor?
